  ...
  return (
    <div ref={sectionRef}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 mb-20 sm:mb-28">
        {metrics.map((m) => (
          <JellyWrapper key={m.label} intensity="soft" className="jelly-card card-metric bg-card text-card-foreground rounded-xl border border-border dark:border-border/50 p-6 text-center card-polished">
      ...
      <div className="grid sm:grid-cols-3 gap-3 sm:gap-5">
        {specializations.map((s, i) => (
          <JellyWrapper key={s.title} intensity="medium" className="jelly-card bg-card text-card-foreground rounded-xl border border-border dark:border-border/50 p-6 h-full card-polished">
